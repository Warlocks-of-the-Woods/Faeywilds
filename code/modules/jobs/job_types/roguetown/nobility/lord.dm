...
/datum/job/roguetown/lord/after_spawn(mob/living/L, mob/M, latejoin = TRUE)
	...
	if(L)
		...
		if(L.gender != FEMALE)
			...
			send2chat(new /datum/tgs_message_content("[L.real_name] - Герцог своего феода."), "launches") // stupid but whatevah. screw you whoever decide to delete config for launchalerts
			addtimer(CALLBACK(L, TYPE_PROC_REF(/mob, lord_color_choice)), 50)
		else
			to_chat(world, "<b><span class='notice'><span class='big'>[L.real_name] is Duchess of the Realm.</span></span></b>")
			send2chat(new /datum/tgs_message_content("[L.real_name] - Герцогиня своего феода."), "launches")
			addtimer(CALLBACK(L, TYPE_PROC_REF(/mob, lord_color_choice)), 50)
