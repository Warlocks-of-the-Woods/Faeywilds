...
/datum/job/roguetown/lord/after_spawn(mob/living/L, mob/M, latejoin = TRUE)
	...
	if(L)
		...
		if(L.gender != FEMALE)
			...
			send2chat(new /datum/tgs_message_content("[L.real_name] - Герцог своего феода."), CONFIG_GET(string/chat_announce_new_game))
			addtimer(CALLBACK(L, TYPE_PROC_REF(/mob, lord_color_choice)), 50)
		else
			to_chat(world, "<b><span class='notice'><span class='big'>[L.real_name] is Duchess of the Realm.</span></span></b>")
			send2chat(new /datum/tgs_message_content("[L.real_name] - Герцогиня своего феода."), CONFIG_GET(string/chat_announce_new_game))
			addtimer(CALLBACK(L, TYPE_PROC_REF(/mob, lord_color_choice)), 50)
